...
public class AudioFrame : MediaFrame
{
    ...
    public int CopyFrom(JaggedSpan2D<float> samples) => CopyFrom<float>(samples);

    /// <inheritdoc cref="CopyFrom(Span{float})"/>
    public int CopyFrom(JaggedSpan2D<short> samples) => CopyFrom<short>(samples);

    public int CopyFrom(JaggedSpan2D<byte> samples) => CopyFrom<byte>(samples);
